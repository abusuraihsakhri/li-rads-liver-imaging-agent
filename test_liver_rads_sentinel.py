import pytest
from liver_rads_sentinel import EnhancementKineticsAgent, MajorFeatureCounterAgent, LIRADSCategoryMatcherAgent, LiverRADSCoordinator, main


def test_sub_agents():
    a1 = EnhancementKineticsAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = MajorFeatureCounterAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = LIRADSCategoryMatcherAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = LiverRADSCoordinator()
...
def test_domain_registry():
    from liver_rads_sentinel import DomainKnowledgeRegistry
